...
![version](https://img.shields.io/badge/version-1.6-00d4ff)
...
- **Оверлей поверх игры** — узкое окно с поиском контрпиков и драфтом.
  ...
  активна игра; повторное нажатие или `Esc` прячет. В драфте враги
  добавляются по одному, по мере пиков, и подбор пересчитывается сразу.
...
- **Подсказки при вводе** — список подходящих героев под полем; стрелки
  ...
  писать по-русски: «пудж» находит Pudge.
...
Клавишу оверлея можно сменить в `overlay_hotkey`: модификаторы `ctrl`, `alt`,
`shift`, `win` и буква, цифра, `f1`–`f24` или `space`. Если комбинацию уже
